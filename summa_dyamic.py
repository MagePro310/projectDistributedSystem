...
# Benchmark function
def benchmark_summa_uneven(max_size, step):
    """
    Benchmark SUMMA with dynamic scaling and uneven chunk handling.
    ...
    for size in range(step, max_size + 1, step):
        M = K = N = size
        A = da.random.random((M, K), chunks=(math.ceil(M / num_workers), K))
        B = da.random.random((K, N), chunks=(K, math.ceil(N / num_workers)))
...
benchmark_results = benchmark_summa_uneven(max_matrix_size, step_size)

# Save results to an Excel file
output_file = "summa_dynamic_benchmark_results_uneven.xlsx"
